...
Документация для команды фронтенда. После ознакомления перейти к [плану работ](https://drive.google.com/drive/folders/1zlyacDdu4jSMTCw9irpVCPFGv3eJJP6H?usp=drive_link) и попытаться что нибудь из этого реализовать. По завершению реализации задачи кидать Pull Request, с заголовком реализованной задачи, в ветку dev.
...
Есть возможность переключаться между разными историями и панелями, например при нажатии на кнопку настроек на нижней панели произойдет переключение с главной истории на историю с настройками, так же измениться отображаемая панель. Модель истории и нижней панели могут быть изменены независимо друг от друга.
...
```java
  ...
  public SourceType status;      // Источник преобразования (Жесты, Звук, Текст)
...
Если что-то не понятно попробуй посмотреть код, поизменять его и посмотреть что будет, так же можно почитать другие полезные доки, например [описание дизайна приложения](design.md), [описание методов интеграции фронта и бэка](front_back_integration.md). Торопиться с реализацией задач не нужно.
