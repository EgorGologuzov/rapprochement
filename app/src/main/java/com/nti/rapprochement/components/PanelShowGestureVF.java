...
public class PanelShowGestureVF extends ViewFactoryBase {
    private final RecordMultiMode model;

    public PanelShowGestureVF(RecordMultiMode model) {
        this.model = model;
    }

    @Override
    public View create(ViewGroup parent) {
        View view = createAndRegister(R.layout.panel_show_gesture, parent);
